...
const request = async (options, showLoading = true) => {
  ...
  options.url = host + '/' + options.url
    ...
    // 参数 url ：接口地址
    // 参数 data ： 请求的参数
    // 参数 header ：请求的 header
    // 参数 method ：请求的方法，有效值：OPTIONS，GET，HEAD，POST，PUT，DELETE，TRACE，CONNECT。
  let response = await wepy.request(options)

    // 隐藏加载中
  if (showLoading) {
        // 与 showLoading 对应，隐藏加载框，注意应该成对出现
    wepy.hideLoading()
  }
...
const login = async (params = {}) => {
  ...
  if (authResponse.statusCode === 201) {
        // 设置缓存中的某个数据; 与之对应的是 getStorageSync ，获取缓存中的某个数据
...
const getToken = async (options) => {
  ...
  let expireAt = wepy.getStorageSync('access_token_expired_at')

    // 如果 token 过期了，则调用刷新方法
  if (accessToken && new Date().getTime() > expireAt) {
